---
title: 我如何用 AI coding agent 接手並交付 NOVA WAREHOUSE 倉儲系統
date: 2026-07-05
summary: 從需求拆解、倉儲後台規則、LINE Flex mock、DevTools 驗收到 GitHub Pages 部署，整理一個可對企業說明的技術案例。
---
...
| 層次 | 拆解結果 |
|---|---|
| 資料 | inventory、orders、report、Flex payload |
| 規則 | 低庫存、缺料阻塞、LINE OA 未出貨 |
| 畫面 | 倉儲後台、訂單可視化、LINE 推播中心 |
| 驗收 | build、mock response、DevTools Network、git diff |

## 3. 我改了什麼功能

在 C2，我透過 Plan Mode 先要求 AI 規劃，再只修改 `web-lab/src/warehouseLogic.js`。目標是新增一條 action queue 規則：

```text
如果存在 LINE OA 通路且尚未出貨的訂單，action queue 要顯示客服確認提醒。
```
